...
import cl.vetnova.soporte.dto.ErrorResponse;
...
public class GlobalExceptionHandlerTest {
    ...
    @Test
    void testRemoteResponde502() {
        ResponseEntity<ErrorResponse> respuesta = handler.remote(new RemoteServiceException("servicio remoto caido"), request);

        assertEquals(502, respuesta.getStatusCode().value());
        assertFalse(respuesta.getBody().success());
        assertNotNull(respuesta.getBody().message());
    }
    ...
    @Test
    void testForbiddenResponde403() {
        ResponseEntity<ErrorResponse> respuesta = handler.forbidden(new ForbiddenException("prohibido"), request);
