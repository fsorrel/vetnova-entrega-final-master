...
public class GlobalExceptionHandler {
    ...
    @ExceptionHandler(RegistroInmutableException.class)
    public ResponseEntity<ErrorResponse> inmutable(RegistroInmutableException ex, HttpServletRequest request) {
        log.warn("event=registro_inmutable path={} message={}", request.getRequestURI(), ex.getMessage());
        return build(HttpStatus.METHOD_NOT_ALLOWED, ex.getMessage(), request.getRequestURI(), Map.of());
    }

    @ExceptionHandler(UnauthorizedException.class)
    public ResponseEntity<ErrorResponse> unauthorized(UnauthorizedException ex, HttpServletRequest request) {
        log.warn("event=unauthorized path={} message={}", request.getRequestURI(), ex.getMessage());
    ...
    public ResponseEntity<ErrorResponse> constraint(ConstraintViolationException ex, HttpServletRequest request) {
        log.warn("event=constraint_violation path={} message={}", request.getRequestURI(), ex.getMessage());
        return build(HttpStatus.BAD_REQUEST, ex.getMessage(), request.getRequestURI(), Map.of());
    }
