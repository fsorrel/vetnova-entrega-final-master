...
public class ErrorResponseTest {
    ...
    void testErrorResponse() {
        ErrorResponse errorResponse = new ErrorResponse(false, "Recurso no encontrado", "/api/v1/notificaciones", 404,
                new HashMap<>(), LocalDateTime.now());
        assertFalse(errorResponse.isSuccess());
        assertEquals("Recurso no encontrado", errorResponse.getMessage());
        assertEquals("/api/v1/notificaciones", errorResponse.getPath());
