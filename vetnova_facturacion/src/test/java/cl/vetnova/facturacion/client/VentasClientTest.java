...
private WebClient webClient;
    ...
    @Test
    void testOrdenConfirmadaDevuelveTrue() {
        VentasClient client = new VentasClient(builderSimulado(), "http://localhost:8084");
        simularGet(Mono.just(Map.of("id", 1L, "estado", "CONFIRMADA")));

        assertTrue(client.ordenConfirmada(1L));
    }

    @Test
    void testOrdenConfirmadaConOtroEstadoDevuelveFalse() {
        VentasClient client = new VentasClient(builderSimulado(), "http://localhost:8084");
        simularGet(Mono.just(Map.of("id", 1L, "estado", "PENDIENTE")));

        assertFalse(client.ordenConfirmada(1L));
    }

    @Test
    void testOrdenConfirmadaConVentasCaidoLanzaRemoteServiceException() {
        VentasClient client = new VentasClient(builderSimulado(), "http://localhost:8084");
        simularGet(Mono.error(new RuntimeException("conexion rechazada")));

        assertThrows(RemoteServiceException.class, () -> client.ordenConfirmada(1L));
    }

    @Test
    void testOrdenConfirmadaConRespuestaVaciaDevuelveFalse() {
        VentasClient client = new VentasClient(builderSimulado(), "http://localhost:8084");
        simularGet(Mono.empty());
